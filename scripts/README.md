...
- `db_cli.py` - Database CLI - консольная утилита для управления БД (просмотр, создание, обновление, удаление)
...
#### Вариант 1: Через SQL-скрипт (рекомендуется для продакшена)

```bash
# Подключиться к PostgreSQL
psql -h localhost -U postgres -d nomus

# Выполнить скрипт инициализации
\i scripts/init_db.sql

# Или из командной строки
psql -h localhost -U postgres -d nomus -f scripts/init_db.sql
```

#### Вариант 2: Через Python (для разработки)

```bash
# Пересоздать все таблицы через SQLAlchemy
poetry run python scripts/recreate_database.py
```

**ВНИМАНИЕ:** Эта команда удалит все данные в таблицах!

### Работа с базой данных (Database CLI)

Интерактивная консольная утилита для управления БД:

```bash
poetry run python scripts/db_cli.py
```

Возможности:
- Просмотр всех пользователей
- Просмотр пользователей с их заказами
